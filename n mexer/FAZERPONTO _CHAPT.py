...
class ProgramaCopiarPeriodo:
    ...
    def copiar_colar(self, data, periodo):
        ...
        # 3️⃣ Inserir linha antes do Total do dia
        self.ws.api.Rows(linha_total_dia).Insert()
        
        # 4️⃣ Determinar última coluna
        ultima_col = self.ws.range("A1").expand("right").last_cell.column

        # 5️⃣ Copiar valores da linha modelo (coluna C em diante)
        for col in range(3, ultima_col + 1):
            valor = self.ws.range((linha_modelo, col)).value
            self.ws.range((linha_total_dia, col)).value = valor

            # Copiar formatação
            cell_modelo = self.ws.range((linha_modelo, col))
            cell_dest = self.ws.range((linha_total_dia, col))
            cell_dest.api.Font.Bold = cell_modelo.api.Font.Bold
            cell_dest.api.Font.Color = cell_modelo.api.Font.Color
            cell_dest.api.Interior.Color = cell_modelo.api.Interior.Color

        # 6️⃣ Atualizar coluna Período (coluna C)
        self.ws.range((linha_total_dia, 3)).value = self.normalizar_periodo(periodo)

        # 7️⃣ Somar linha no Total do dia
        self.somar_linha_no_total_do_dia(linha_total_dia, linha_total_dia)

        # 8️⃣ Somar linha no Total Geral
        self.somar_linha_no_total_geral(linha_total_dia)

        print(f"➕ Linha {linha_total_dia} adicionada e somada ao TOTAL DO DIA e TOTAL GERAL")
