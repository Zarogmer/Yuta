    ...
    def executar(self):
        ...
        def copiar_para_temp_e_ler_excel(caminho_original: Path | str) -> pd.DataFrame:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                print(
                    f"Copiando {caminho_original.name} para pasta temporária local..."
                )
                shutil.copy2(caminho_original, caminho_temp)

                print(f"Lendo arquivo temporário: {caminho_temp}")
                df = pd.read_excel(
                    caminho_temp, engine="openpyxl"
                )  # engine explícito ajuda
        ...
        def obter_pasta_faturamentos() -> Path:
                ...
                if base.exists():
                    ...
                    for candidato in candidatos:
                        ...
                        if "01. FATURAMENTOS" in candidato.parent.as_posix():
                            ...
                            print(
                                f"✅ Pasta FATURAMENTOS encontrada em:\n   {caminho_alvo}"
                            )
                            break
            ...
            if not caminho_alvo:
                ...
                raise FileNotFoundError(
                    "Pasta FATURAMENTOS não localizada automaticamente"
                )
            ...
            for arq in sorted(caminho_alvo.glob("*.xlsx"))[
                :10
        ...
        def montar_datas_report_vigia(
            ...
            if periodos is None:
                raise ValueError(
                    "É necessário informar 'periodos' para preencher as datas"
                )

            data_inicio, data_fim = obter_datas_extremos(ws_resumo)
            if not data_inicio or not data_fim:
                raise ValueError(
                    "Não foi possível determinar as datas extremas na aba RESUMO"
                )
        ...
        def cargonave(ws):
            ...
            return (
                str(valor_c9).strip().upper() == "A/C AGÊNCIA MARÍTIMA CARGONAVE LTDA."
            )
...
class FaturamentoDeAcordo:
    ...
    def executar(self):
        print("🚀 Iniciando Faturamento De Acordo - SANPORT\n")

        try:
            # 1. Busca automática da pasta FATURAMENTOS (igual à opção completa)
            pasta_faturamentos = obter_pasta_faturamentos()

            # 2. Usa exatamente a mesma função da opção completa para abrir os arquivos
            resultado = abrir_workbooks(pasta_faturamentos)
            ...
            try:
                # 3. Extrai DN e nome do navio da pasta do navio (pasta do 1.xlsx)
                pasta_navio = Path(wb1.fullname).parent
                dn = obter_dn_da_pasta(pasta_navio)
                nome_navio = obter_nome_navio_da_pasta(pasta_navio)

                print(f"📋 DN identificada: {dn}")
                print(f"🚢 Navio: {nome_navio}")

                # 4. Preenche a aba FRONT VIGIA
                self._preencher_front_vigia(ws_front, dn, nome_navio)

                # 5. Remove todas as abas exceto a FRONT VIGIA
                nome_aba_front = ws_front.name
                for sheet in list(wb2.sheets):
                    if sheet.name != nome_aba_front:
                        sheet.delete()

                # 6. Salva na Área de Trabalho
                ...
                # 7. Exporta PDF com layout ajustado
                print("📄 Gerando PDF...")
                ...
                print(f"\n✅ FATURAMENTO DE ACORDO CONCLUÍDO COM SUCESSO!")
    ...
    # Método interno para preencher a FRONT VIGIA (separado para ficar limpo)
    def _preencher_front_vigia(self, ws_front, dn, nome_navio):
        ...
        print("🧹 Célula G27 limpa")

        # Regra especial Unimar
        cliente_c9 = str(ws_front.range("C9").value or "").strip()
        if "Unimar Agenciamentos" in cliente_c9:
            ws_front.range("G26").value = 400
            print("⚠️ Cliente Unimar detectado → G26 = 400")

        # Taxa administrativa +20
        try:
            valor_atual = float(ws_front.range("C35").value or 0)
            ws_front.range("C35").value = valor_atual + 20
        except:
            ws_front.range("C35").value = 20
        print("💰 Taxa administrativa +20 aplicada em C35")

        # Rodapé
        ws_front.range("C39").value = f"Santos, {hoje.day} de {meses[hoje.month]} de {hoje.year}"

        print("✅ FRONT VIGIA preenchida com sucesso!")
        ...
        while True:
            ...
            if opcao == "1":
                ...
                try:
                    ...
                    )  # Pausa pra você ler o resultado
                ...
                try:
                    ...
                    input(
                        "Pressione ENTER para continuar..."
                    )  # Pausa pra você ver que terminou
